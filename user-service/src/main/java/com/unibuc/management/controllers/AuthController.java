...
public class AuthController {
    ...
    /**
     * Internal endpoint invoked by medical-service (via Feign + JWT) when a
     * doctor/patient profile is deleted there. Removes only the user account.
     */
    @DeleteMapping("/internal/users/{userId}")
    public ResponseEntity<Void> deleteUserInternal(@PathVariable Long userId) {
        authService.deleteUserInternal(userId);
        return ResponseEntity.noContent().build();
    }
}
