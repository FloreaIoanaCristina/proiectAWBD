...
class AppointmentServiceTest {
    ...
    private static Collection<GrantedAuthority> roles(String role) {
    ...
    void findById_shouldReturnAppointment() {
        ...
        Optional<AppointmentResponseDTO> result = appointmentService.findById(10);
    ...
    void findById_shouldReturnEmptyOptional() {
        ...
        Optional<AppointmentResponseDTO> result = appointmentService.findById(99);
    ...
    void getAppointmentsByPatientIdPaged_shouldReturnPage() {
        ...
        when(appointmentRepository.findByPatientId(1, pageable)).thenReturn(page);
        ...
        verify(appointmentRepository).findByPatientId(1, pageable);
    }
    ...
    void getAppointmentsByDoctorIdPaged_shouldReturnPage() {
        ...
        appointment.setId(5);
        Page<Appointment> page = new PageImpl<>(List.of(appointment));
        when(appointmentRepository.findByDoctorId(2, pageable)).thenReturn(page);
        ...
        verify(appointmentRepository).findByDoctorId(2, pageable);
    }
    ...
    void submitFeedback_shouldUpdateRatingAndCompleteAppointment() {
        ...
        appointment.setAppointmentFrom(OffsetDateTime.now().minusHours(1));
        ...
        assertEquals(4.545454545454546, medicalService.getRating());
    ...
    void submitFeedback_shouldThrowWhenPatientNotFound() {
        ...
        when(appointmentRepository.findById(1)).thenReturn(Optional.of(appointment));
        when(patientRepository.findByUsername("patient")).thenReturn(Optional.empty());
    ...
    void submitFeedback_shouldThrowWhenAppointmentBelongsToAnotherPatient() {
        ...
        when(appointmentRepository.findById(1)).thenReturn(Optional.of(appointment));
        when(patientRepository.findByUsername("patient")).thenReturn(Optional.of(anotherPatient));
    ...
    void submitFeedback_shouldThrowWhenAppointmentNotFinished() {
        ...
        appointment.setPatient(patient);
        appointment.setMedicalService(medicalService);
        appointment.setAppointmentFrom(OffsetDateTime.now().minusMinutes(10));
    ...
    void getAvailableTimeSlots_shouldReturnAllSlotsWhenNothingBlocksThem() {
        ...
        when(appointmentRepository.findByMedicalServiceAndDate(eq(1), any(), any())).thenReturn(List.of());
        when(ptoRepository.findActivePtoForDoctorInDay(eq(2), any(), any())).thenReturn(List.of());

        String date = OffsetDateTime.now(ZoneOffset.UTC).plusDays(1).toLocalDate().toString();

        List<OffsetDateTime> result = appointmentService.getAvailableTimeSlots(1, 2, date);
    ...
    void getAvailableTimeSlots_shouldExcludeExistingAppointment() {
        ...
        String date = OffsetDateTime.now(ZoneOffset.UTC).plusDays(1).toLocalDate().toString();
        OffsetDateTime occupiedSlot = LocalDate.parse(date).atTime(8, 30).atOffset(ZoneOffset.UTC);
        ...
        when(appointmentRepository.findByMedicalServiceAndDate(eq(1), any(), any())).thenReturn(List.of(appointment));
        when(ptoRepository.findActivePtoForDoctorInDay(eq(2), any(), any())).thenReturn(List.of());
    ...
    void getAvailableTimeSlots_shouldExcludePtoSlots() {
        ...
        pto.setPtoFrom(LocalDate.parse(date).atTime(8, 30).atOffset(ZoneOffset.UTC));
        pto.setPtoTo(LocalDate.parse(date).atTime(9, 30).atOffset(ZoneOffset.UTC));

        when(ptoRepository.findActivePtoForDoctorInDay(eq(2), any(), any())).thenReturn(List.of(pto));

        List<OffsetDateTime> result = appointmentService.getAvailableTimeSlots(1, 2, date);

        assertEquals(2, result.size());
        assertFalse(result.contains(LocalDate.parse(date).atTime(8, 30).atOffset(ZoneOffset.UTC)));
        assertFalse(result.contains(LocalDate.parse(date).atTime(9, 0).atOffset(ZoneOffset.UTC)));
    }
    ...
    void getAvailableTimeSlots_shouldWorkWithoutDoctor() {
        ...
        List<OffsetDateTime> result = appointmentService.getAvailableTimeSlots(1, null, date);

        assertEquals(2, result.size());
        verify(ptoRepository, never()).findActivePtoForDoctorInDay(any(), any(), any());
    }
    ...
    void updateAppointment_shouldUpdateAppointment() {
        ...
        doReturn(roles("ROLE_PATIENT")).when(authentication).getAuthorities();
        when(patientRepository.findByUsername("patient")).thenReturn(Optional.of(patient));
        when(medicalServiceService.getMedicalServiceById(1)).thenReturn(medicalService);
        when(appointmentRepository.findByMedicalServiceAndDate(eq(1), any(), any())).thenReturn(List.of());
        when(appointmentRepository.save(any(Appointment.class))).thenAnswer(invocation -> invocation.getArgument(0));

        AppointmentResponseDTO updated = appointmentService.updateAppointment(1, newDate, authentication);
    ...
    void updateAppointment_shouldThrowWhenDateIsInPast() {
        ...
        assertThrows(InvalidActionException.class,
                () -> appointmentService.updateAppointment(1, yesterday, authentication));
    ...
    void updateAppointment_shouldThrowWhenAppointmentNotFound() {
        ...
        assertThrows(ResourceNotFoundException.class,
                () -> appointmentService.updateAppointment(1, tomorrow, authentication));
    ...
    void updateAppointment_shouldThrowWhenSlotUnavailable() {
        ...
        when(appointmentRepository.findByMedicalServiceAndDate(eq(1), any(), any())).thenReturn(List.of(occupied));
    ...
    void getAppointmentsByPatientId_shouldReturnAppointmentsForOwnerPatient() {
        ...
        Page<AppointmentResponseDTO> result =
                appointmentService.getAppointmentsByPatientId(1, pageable, authentication);
    ...
    void getAppointmentsByPatientId_shouldFilterAppointmentsForDoctor() {
        ...
        doReturn(roles("ROLE_DOCTOR")).when(authentication).getAuthorities();
        when(doctorService.getDoctorByUsername("doctor")).thenReturn(currentDoctor);
        when(appointmentRepository.findByPatientId(1, pageable)).thenReturn(page);
    ...
    void getAppointmentsByDoctorId_shouldReturnAppointments() {
        ...
        when(appointmentRepository.findByDoctorId(5, pageable)).thenReturn(page);

        Page<AppointmentResponseDTO> result =
                appointmentService.getAppointmentsByDoctorId(5, pageable, authentication);

        assertEquals(1, result.getTotalElements());
        assertEquals(1, result.getContent().get(0).getId());
        verify(appointmentRepository).findByDoctorId(5, pageable);
    }
    ...
    void getAppointmentsByDoctorId_shouldThrowWhenAccessingAnotherDoctor() {
        ...
        assertThrows(UnauthorizedAccessException.class,
                () -> appointmentService.getAppointmentsByDoctorId(10, pageable, authentication));

        verify(appointmentRepository, never()).findByDoctorId(anyInt(), any());
    }
    ...
    void createAppointment_shouldCreateAppointment_whenPatientRole() {
        ...
        doReturn(List.of(slot)).when(appointmentService).getAvailableTimeSlots(anyInt(), any(), any());
        when(paymentClient.createForAppointment(any())).thenReturn(new PaymentSummaryDTO());
    ...
    void createAppointment_shouldThrow_whenSlotNotAvailable() {
        ...
        doReturn(List.of()).when(appointmentService).getAvailableTimeSlots(anyInt(), any(), any());
