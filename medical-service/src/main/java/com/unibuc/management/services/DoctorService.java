...
public class DoctorService {
    ...
    public DoctorResponseDTO createDoctor(DoctorRequestDTO doctorRequestDTO) {
        ...
        if (doctorRequestDTO.getUserId() != null) {
            doctor.setUserId(doctorRequestDTO.getUserId());
    ...
    public void deleteDoctor(Integer id) {
        ...
        if (userId != null) {
            ...
            if (isDoctor && !currentUsername.equals(doctor.getUsername())) {
                throw new InvalidActionException(
                        "Nu aveți permisiunea să ștergeți profilul unui alt medic care are cont activ.");
            }

            doctorRepository.delete(doctor);
            log.info("Se șterge utilizatorul asociat (ID: {}) în cascadă via user-service.", userId);
    ...
    public void deleteByUserId(Long userId) {
        doctorRepository.findByUserId(userId).ifPresent(doctor -> {
            ...
            doctorRepository.delete(doctor);
            log.info("Profilul de doctor asociat User ID {} a fost șters.", userId);
        });
    }
